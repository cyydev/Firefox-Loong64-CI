...
function logPrint() {
  ...
  status=$3
  echo "[[$date  LINE:$lineNo]]----$messages" | tee -a $LOG_CI_FILE
...
function testToolsInstalled(){
  ...
  then
    ...
    sudo apt-get install sendmail mail mutt # email tools.
...
function copyPackage() {
  ...
  pushd $objdir
    ...
    if [ $? != 0 ]
    then
      logPrint $LINENO "Backed Up Failed. Please Check!" "ERROR"
    fi
